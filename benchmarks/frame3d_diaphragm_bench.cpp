...
static int mid(int s){return 900000+s;}
...
static CompiledFrame3D make_space_frame(int stories,int nx,int ny){
    ...
    for(int s=0;s<=stories;++s){
        ...
        if(s>0){
            b.add_node(mid(s),0.5*nx*L,0.5*ny*L,s*h);
            b.fix_dof(mid(s),Dof3D::UZ); b.fix_dof(mid(s),Dof3D::RX); b.fix_dof(mid(s),Dof3D::RY);
            std::vector<int> slaves; for(int ix=0;ix<=nx;++ix)for(int iy=0;iy<=ny;++iy)slaves.push_back(nid(s,ix,iy));
            b.rigid_diaphragm_z(mid(s),slaves);
        }
    }
    ...
    b.set_rayleigh(0.02,0.0005); b.set_ground_direction(Dof3D::UX); b.set_response(mid(stories),Dof3D::UX);
    std::vector<int> stories_ids; for(int s=1;s<=stories;++s) stories_ids.push_back(mid(s)); b.set_story_nodes(std::move(stories_ids),Dof3D::UX);
    return b.compile();
}
...
int main(int argc,char** argv){
    const int stories=argc>1?std::stoi(argv[1]):10, nx=argc>2?std::stoi(argv[2]):2, ny=argc>3?std::stoi(argv[3]):2, steps=argc>4?std::stoi(argv[4]):300;
    const double amp=argc>5?std::stod(argv[5]):4.0, dt=0.005;
    ...
    std::cout<<"rigid_diaphragm=1 stories="<<stories<<" bays_x="<<nx<<" bays_y="<<ny<<" dof="<<model.dof()<<" mass_nnz="<<model.mass_matrix().nnz()<<" elastic_elements="<<model.elastic_element_count()<<" nonlinear_springs="<<model.nonlinear_count()<<" basis_nnz="<<model.nonlinear_basis().nnz()<<" steps="<<steps<<"\n";
