...
RunOut run15(const ModelBuild& mb,const Motion& motion){
    ...
    r.analysis=run_newmark_robust(mb.model,motion.ag,motion.dt,LinearStrategy::FullFactorization,ro);return r;
}
}
int main(int argc,char**argv){
    try{
        const std::string motion_path=argc>1?argv[1]:"validation/uc_berkeley_3story/proxy_dt1_motion.csv";
        const std::string out_json=argc>2?argv[2]:"validation/uc_berkeley_3story/phase9/phase9f2_mroz_15s.json";
        const std::string out_csv=argc>3?argv[3]:"validation/uc_berkeley_3story/phase9/phase9f2_mroz_15s_history.csv";
        ...
        const auto gravity=gravity_column_preloads();constexpr double flex_scale=0.934616,target_T=.48,slope=-.005,residual=.20,acc=.55;const double w=2*M_PI/target_T,alpha=2*.025*w,beta=2*.005/w;
        ...
        std::cout<<out_json<<"\n"<<out_csv<<"\n";return 0;
    }catch(const std::exception&e){std::cerr<<"9F2 error: "<<e.what()<<"\n";return 2;}
}
